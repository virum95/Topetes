...
public class VentanaPrincipalDos {
	...
	public VentanaPrincipalDos(JFrame frame) {
		...
		final Cursor c1 = toolkit.createCustomCursor(cursor, new Point(7, 25),
				"img");
		...
		for (int i = 0; i < arrayAnimales.length; i++) {
			for (int j = 0; j < arrayAnimales[i].length; j++) {
				...
				if (arrayAnimales[i][j] instanceof Topete) {
					final int a = i;
					final int b = j;
					arrayAnimales[i][j].getImg().addMouseListener(
							new MouseAdapter() {
	 ...
	 * 
	 * @return true - Todas las casillas llenas false - Hay alguna casilla que
	 *         no esta llena
	 */
	...
	/**
	 * Clase que permite lanzar un hilo para introducir todos los animales en la
	 * ventana. Se hace en un hilo para que el hilo principal espere a que esten
	 * todos creados
	 * 
	 * @author Gaizka
	 *
		...
		public void run() {
			...
			for (int i = 0; i < arrayPaneles.length; i++) {
				for (int j = 0; j < arrayPaneles[i].length; j++) {
					...
					switch (i) {
					case 0:
						arrayPaneles[i][j].setBounds(X_EJE_UNO, Y_EJE_UNO, 123,
								141);
						break;
					case 1:
						arrayPaneles[i][j].setBounds(X_EJE_DOS, Y_EJE_UNO, 123,
								141);
						break;
					case 2:
						arrayPaneles[i][j].setBounds(X_EJE_TRES, Y_EJE_UNO,
								123, 141);
						break;
					case 3:
						arrayPaneles[i][j].setBounds(X_EJE_UNO, Y_EJE_DOS, 123,
								141);
						break;
					case 4:
						arrayPaneles[i][j].setBounds(X_EJE_DOS, Y_EJE_DOS, 123,
								141);
						break;
					case 5:
						arrayPaneles[i][j].setBounds(X_EJE_TRES, Y_EJE_DOS,
								123, 141);
						break;
					case 6:
						arrayPaneles[i][j].setBounds(X_EJE_UNO, Y_EJE_TRES,
								123, 141);
						break;
					case 7:
						arrayPaneles[i][j].setBounds(X_EJE_DOS, Y_EJE_TRES,
								123, 141);
						break;
					case 8:
						arrayPaneles[i][j].setBounds(X_EJE_TRES, Y_EJE_TRES,
								123, 141);
						break;
					case 9:
						arrayPaneles[i][j].setBounds(X_EJE_UNO, Y_EJE_CUATRO,
								123, 141);
						break;
					case 10:
						arrayPaneles[i][j].setBounds(X_EJE_DOS, Y_EJE_CUATRO,
								123, 141);
						break;
					case 11:
						arrayPaneles[i][j].setBounds(X_EJE_TRES, Y_EJE_CUATRO,
								123, 141);
						break;
					default:
						break;
	...
	/*
	 * Hilo que se ocupa de mover los tops/gatos de la posición inicial hasta
	 * arriba
	 */
	class MiRunnable2 implements Runnable {
		...
		/**
		 * Método que hace que salga un animal. Elige aleatoriamente uno de los
		 * topos Las posibilidades son: 60% para el topo normal 20% para el topo
		 * con casco 10% para el gato y para el topo negro
		 */
		public void creaAnimal() {
			...
			do {
				...
				i = r.nextInt(12);
			} while (getOcupado()[i]); // Evita que si ya hay un topo en el
										// espacio seleccionado, se cree otro
			saleTopo(i, j);
			ocupado[i] = true;
			getArrayAnimales()[i][j].setFechaCreacion(System
					.currentTimeMillis());
			getArrayAnimales()[i][j].setFuera(true);
		}

		/**
		 * Método que hace que los topos se muevan hacia arriba. LLama a un
		 * metodo recursivo para que el movimiento sea poco a poco
		 * 
		 * @param j
		 */
		...
		/*
		 * Método recursivo que es llamado desde saleTopo para que la salida sea
		 * más suave
		 */
	...
	/*
	 * Hilo que se encarga de meter los topos cuando se clican o cuando pasa un
	 * tiempo determinado
	 */
	class MiRunnable3 implements Runnable {
		 ...
		 * 
		 * @param panel
		 *            El panel en el que se encuentra el animal a quitar
		 * @param animal
		 *            El animal que se quiere quitar
		 */
		...
		/*
		 * Método que llama a un método recursivo que se encarga de meter el
		 * topo o el gato
		 */
		...
		/*
		 * Método que carga el resultado(puntuación) en la base de datos con un
		 * nombre
		 */
		...
		public void run() {
			while (sigue) {
				if (!estamosLlenos()) {
					for (int i = 0; i < arrayAnimales.length; i++) {
						for (int j = 0; j < arrayAnimales[i].length; j++) {
							// Si el topo está más tiempo del que puede estar
							// fuera
							if (getArrayAnimales()[i][j].getFechaCreacion() != 0) {
								if (System.currentTimeMillis()
										- getArrayAnimales()[i][j]
												.getFechaCreacion() >= (TIEMPO_FUERA_TOPO * 1000 - puntuacion * 2)) {
									quitaAnimal(i, j); // Quitamos el topo
									getArrayAnimales()[i][j]
											.setFechaCreacion(0); // Ponemos la
																	// fecha de
																	// creacion
																	// a 0
									// Sumamos uno a los eliminados si son menos
									// de dos
